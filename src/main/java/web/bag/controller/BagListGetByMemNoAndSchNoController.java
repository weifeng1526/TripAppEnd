...
@WebServlet("/bag/getitems")
public class BagListGetByMemNoAndSchNoController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (memNoStr == null || schNoStr == null) {
				resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				resp.getWriter().write("Missing parameters: memNo or schNo");
				return;
			}
			// 解析參數為整數
			...
			List<BagList> oneBagLists = dao.getBagListByMemNoAndSchNo(memNo, schNo);
			Gson gson = new Gson();
			...
			resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			resp.getWriter().write("錯誤格式 for memNo or schNo");
		}
	}
}
